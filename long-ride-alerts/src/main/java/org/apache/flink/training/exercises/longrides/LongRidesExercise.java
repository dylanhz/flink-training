...
public class LongRidesExercise {
    ...
    public static class AlertFunction extends KeyedProcessFunction<Long, TaxiRide, Long> {
        ...
        // TODO Consider unordered case.

        private static final long MAX_DURATION = Duration.ofHours(2).toMillis();
        ...
        public void open(Configuration config) {
            valueState =
                    getRuntimeContext()
                            .getState(new ValueStateDescriptor<>("valueState", TaxiRide.class));
        }

        @Override
        public void processElement(TaxiRide ride, Context context, Collector<Long> out)
                throws Exception {
            TaxiRide value = valueState.value();
            if (value == null) {
                valueState.update(ride);
                if (ride.isStart) {
                    context.timerService()
                            .registerEventTimeTimer(ride.getEventTimeMillis() + MAX_DURATION);
                }
            } else {
                if (ride.isStart) {
                    if (rideTooLong(ride, value)) {
                        out.collect(ride.rideId);
                    }
                } else {
                    context.timerService()
                            .deleteEventTimeTimer(value.getEventTimeMillis() + MAX_DURATION);
                    if (rideTooLong(value, ride)) {
                        out.collect(ride.rideId);
                    }
                }
                valueState.clear();
        ...
        private boolean rideTooLong(TaxiRide start, TaxiRide end) {
            return start.getEventTimeMillis() + MAX_DURATION < end.getEventTimeMillis();
        }
    }
}
